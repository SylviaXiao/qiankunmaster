...
import {
  financeExpenseFlowStatistics,
  getSysFeeItem,
  financeExpenseFlowStatisticsByCostClassify,
  listAllOrgDeptTreeNoSchoolId,
  financeExpenseFlowStatisticsDetails
...
export default {
  ...
  watch: {
    $route: {
      handler: function(route) {
        if (route.name == 'financeExpenseFlowStatisticsDetail') {
          let ids = localStorage.getItem('financeExpenseFlowStatisticsDetailId')
          ...
          //设置打印链接
          if (this.feeType == 'A') this.exportUrl = '/finspendingFinance/capitalDaily/financeExpenseFlowStatisticsByExportExcel'
          if (this.feeType == 'B') this.exportUrl = '/finspendingFinance/capitalDaily/financeExpenseFlowStatisticsByCostClassifyAndExportExcel'
          if (this.feeType == 'C') this.exportUrl = '/finspendingFinance/capitalDaily/financeExpenseFlowStatisticsDetailsByExportExcel'
  ...
  data() {
    return {
      ...
      level: 'A', //组织层级
      feeType: 'A', //归类层级
      exportUrl: '/finspending/capitalDaily/financeExpenseFlowStatisticsByExportExcel',
      //表头
      headData: [
        {
  ...
  methods: {
    toCostPricePrice(record) {
      let { deptKey,lastType, feeName, totalPrice,price, paymentDate, bankId } = record
      if(lastType==='out'||lastType==='in'){
        this.$router.push({
      ...
      if (data.isClick) {
        ...
        if ((key == 'col' || key == 'colTotal') && !data.isChildren) {
          if (key == 'col' && this.loadData.length > 2) {
            ...
            localStorage.setItem('financeExpenseFlowStatisticsDetailId', data.deptId)
            //结合open打开新的页面
        ...
        //   id: id
        // }
        localStorage.setItem('financeExpenseFlowStatisticsDetailId', id)
        //结合open打开新的页面
        window.open(href, '_blank')
        // this.$router.push({
        //   name: 'financeExpenseFlowStatisticsDetail',
    ...
    async init() {
      ...
      let request = financeExpenseFlowStatistics
      if (this.feeType == 'B') request = financeExpenseFlowStatisticsByCostClassify
      if (this.feeType == 'C') request = financeExpenseFlowStatisticsDetails
        ...
        rows.forEach(item => {
          ...
          headerOrderList.forEach((col, colIndex) => {
            data.push({
              ...
              price:Math.abs(item.totalPrice)
            })
          })
          loadData.push({
            style: 'background:#fff;',
