<template>
  <div class="fin-profit-paytype-wrapper">
    <ReportTable
      ...
      :loadData="loadData"
      :exportUrl="'/finance/spending/financialExpenseFlowListByExportExcel'"
    ></ReportTable>
  </div>
</template>
...
import { financialExpenseFlowList } from '@/api/table/table'
import Vue from 'vue'
const date = new Date()
...
export default {
  ...
  methods: {
    ...
    async init(data) {
      ...
      let res = await financialExpenseFlowList(data)
      //初始化表格内容
    ...
    searchSubmit(data) {
      let newdata = Object.assign(this.queryParam, data)
      let { deptIds, month } = newdata
