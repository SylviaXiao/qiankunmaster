...
import { financeIncomeFirstColumns, pageSizeOptions } from '@/utils/tableDetails/details'
import { STable } from '@/components'
import { financialExpenseFlowListDetails } from '@/api/table/table'
export default {
  ...
  data() {
    return {
      ...
      columns: financeIncomeFirstColumns,
      queryParam: {},
      loadData: parameter => {
        ...
        let request = financialExpenseFlowListDetails
  ...
  watch: {
    $route: {
      handler: function(route) {
        if (route.name == 'financeIncomeDetail') {
  ...
  methods: {
    //导出
    downloadStu() {
      let exportUrl = '/finance/spending/financialExpenseFlowListDetailsByExportExcel'
