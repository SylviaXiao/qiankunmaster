...
import {
  costExpenseFlowStatisticsSplit,
  getSysFeeItem,
  costExpenseFlowStatisticsSplitByCostClassify,
  listAllOrgDeptTreeNoSchoolId,
  costExpenseFlowStatisticsSplitDetails
...
export default {
  ...
  watch: {
    $route: {
      handler: function(route) {
        if (route.name == 'costExpenseFlowStatisticsSplitDetail') {
          let ids = localStorage.getItem('costExpenseFlowStatisticsSplitDetailId')
          ...
          //设置打印链接
          if (this.feeType == 'A') this.exportUrl = '/finspendingCost/capitalDaily/costExpenseFlowStatisticsSplitByExportExcel'
          if (this.feeType == 'B') this.exportUrl = '/finspendingCost/capitalDaily/costExpenseFlowStatisticsSplitByCostClassifyAndExportExcel'
          if (this.feeType == 'C') this.exportUrl = '/finspendingCost/capitalDaily/costExpenseFlowStatisticsSplitDetailsByExportExcel'
  ...
  data() {
    return {
      ...
      level: 'A', //组织层级
      feeType: 'A', //归类层级
      exportUrl: '/finspendingCost/capitalDaily/costExpenseFlowStatisticsSplitByExportExcel',
      //表头
      headData: [
        {
  ...
  methods: {
      ...
      if (data.isClick) {
        ...
        if ((key == 'col' || key == 'colTotal') && !data.isChildren) {
          if (key == 'col' && this.loadData.length > 2) {
            ...
            localStorage.setItem('costExpenseFlowStatisticsSplitDetailId', data.deptId)
            //结合open打开新的页面
        ...
        //   id: id
        // }
        localStorage.setItem('costExpenseFlowStatisticsSplitDetailId', id)
        //结合open打开新的页面
        window.open(href, '_blank')
        // this.$router.push({
        //   name: 'costExpenseFlowStatisticsSplitDetail',
    ...
    async init() {
      ...
      let request = costExpenseFlowStatisticsSplit
      if (this.feeType == 'B') request = costExpenseFlowStatisticsSplitByCostClassify
      if (this.feeType == 'C') request = costExpenseFlowStatisticsSplitDetails
