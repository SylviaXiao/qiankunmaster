...
import {
  operatingExpenseFlowStatistics,
  getSysFeeItem,
  operatingExpenseFlowStatisticsByCostClassify,
  listAllOrgDeptTreeNoSchoolId,
  operatingExpenseFlowStatisticsDetails
...
function getFeeItemName(item) {
  return item.feeItemName + '-' + item.operateName
}
const searchParams = [
  {
    type: 'date',
...
export default {
  ...
  watch: {
    $route: {
      handler: function(route) {
        if (route.name==='dailyStatementOfCapitalDetail') {
          let ids = localStorage.getItem('dailyStatementOfCapitalDetailId')
          ...
          //设置打印链接
          if (this.feeType==='A') this.exportUrl = '/finspending/capitalDaily/operatingExpenseFlowStatisticsByExportExcel'
          if (this.feeType==='B') this.exportUrl = '/finspending/capitalDaily/operatingExpenseFlowStatisticsByCostClassifyAndExportExcel'
          if (this.feeType==='C') this.exportUrl = '/finspending/capitalDaily/operatingExpenseFlowStatisticsDetailsByExportExcel'
  ...
  data() {
    return {
      ...
      level: 'A', //组织层级
      feeType: 'A', //归类层级
      exportUrl: '/finspending/capitalDaily/operatingExpenseFlowStatisticsByExportExcel',
      //表头
      headData: [
        {
  ...
  methods: {
    ...
    searchSubmit(data) {
      ...
      if (itemNames) {
        let feeNameList = itemNames.split(',').map(item => {
          return {
            ...
            operateName: item.split('-')[1]
          }
        })
        feeName = feeNameList.map(item => item.feeName).join(',')
        operateName = Array.from(new Set(feeNameList.map(item => item.operateName))).join(',')
      }
      ...
      params.costClassify = operateName || costClassify
      ...
      if (data.isClick) {
        ...
        if ((key==='col' || key==='colTotal') && !data.isChildren) {
          if (key==='col' && this.loadData.length > 2) {
            ...
            localStorage.setItem('dailyStatementOfCapitalDetailId', data.deptId)
            //结合open打开新的页面
        ...
        //   id: id
        // }
        localStorage.setItem('dailyStatementOfCapitalDetailId', id)
        //结合open打开新的页面
        window.open(href, '_blank')
        // this.$router.push({
        //   name: 'dailyStatementOfCapitalDetail',
    ...
    async init() {
      ...
      let request = operatingExpenseFlowStatistics
      if (this.feeType==='B') request = operatingExpenseFlowStatisticsByCostClassify
      if (this.feeType === 'C') request = operatingExpenseFlowStatisticsDetails
