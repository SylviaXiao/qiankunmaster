...
import {
  financeExpenseFlowStatisticsSplit,
  getSysFeeItem,
  financeExpenseFlowStatisticsSplitByCostClassify,
  listAllOrgDeptTreeNoSchoolId,
  financeExpenseFlowStatisticsSplitDetails
...
function getFeeItemName(item) {
  return item.feeItemName + '-' + item.financeName
}
const searchParams = [
  {
    type: 'date',
...
export default {
  ...
  watch: {
    $route: {
      handler: function(route) {
        if (route.name == 'financeExpenseFlowStatisticsSplitDetail') {
          let ids = localStorage.getItem('financeExpenseFlowStatisticsSplitDetailId')
          ...
          //设置打印链接
          if (this.feeType == 'A') this.exportUrl = '/finspendingFinance/capitalDaily/financeExpenseFlowStatisticsSplitByExportExcel'
          if (this.feeType == 'B') this.exportUrl = '/finspendingFinance/capitalDaily/financeExpenseFlowStatisticsSplitByCostClassifyAndExportExcel'
          if (this.feeType == 'C') this.exportUrl = '/finspendingFinance/capitalDaily/financeExpenseFlowStatisticsSplitDetailsByExportExcel'
  ...
  data() {
    return {
      ...
      level: 'A', //组织层级
      feeType: 'A', //归类层级
      exportUrl: '/finspendingFinance/capitalDaily/financeExpenseFlowStatisticsSplitByExportExcel',
      //表头
      headData: [
        {
  ...
  methods: {
    toCostPricePrice(record) {
      let { date, finDeptId, deptNo, feeName, price, splitDate, bankId } = record
      // return
      this.$router.push({
    ...
    searchSubmit(data) {
      ...
      let params = {}
      let { Month, deptIds, costClassify, itemNames, projectName, level, startMonth, endMonth } = data
      ...
      if (itemNames) {
        let feeNameList = itemNames.split(',').map(item => {
          return {
            ...
            financeName: item.split('-')[1]
          }
        })
        feeName = feeNameList.map(item => item.feeName).join(',')
        financeName = Array.from(new Set(feeNameList.map(item => item.financeName))).join(',')
      }
      ...
      params.costClassify = financeName || costClassify
      ...
      if (data.isClick) {
        ...
        if ((key == 'col' || key == 'colTotal') && !data.isChildren) {
          if (key == 'col' && this.loadData.length > 2) {
            ...
            localStorage.setItem('financeExpenseFlowStatisticsSplitDetailId', data.deptId)
            //结合open打开新的页面
        ...
        console.log(77777, { type: type, level: level, startDate: startDate, endDate: endDate })
        // return
        const { href } = this.$router.resolve({
          name: 'financeExpenseFlowStatisticsSplitDetail',
          params: { type: type, level: level, startDate: startDate, endDate: endDate }
        ...
        //   id: id
        // }
        localStorage.setItem('financeExpenseFlowStatisticsSplitDetailId', id)
        //结合open打开新的页面
        window.open(href, '_blank')
        // this.$router.push({
        //   name: 'financeExpenseFlowStatisticsSplitDetail',
    ...
    async init() {
      ...
      let request = financeExpenseFlowStatisticsSplit
      if (this.feeType == 'B') request = financeExpenseFlowStatisticsSplitByCostClassify
      if (this.feeType == 'C') request = financeExpenseFlowStatisticsSplitDetails
