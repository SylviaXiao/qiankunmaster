  ...
  <div class='teacher-demand-wrapper'>
      ...
      <div class='btnGroup' style='margin-bottom: 15px;'>
        ...
        <div style="text-align:right; margin: 10px 0; " ref="elementBtn">
          <a-radio-group @change="changeCheck" :default-value="socialStatus"  v-model="socialStatus">
              <a-radio-button value="A">待减员</a-radio-button>
            <a-radio-button value="B">已减员</a-radio-button>
      ...
      <s-table ref='table' size='default' :columns='columns' :data='loadData' rowKey='id'>
        <span slot='action' slot-scope='text, record'>
          <perm-box perm='salary:socialSecurity:save' v-if='record.type==="A"'>
            <a href='#' @click='handleApply(record,true)'>停保</a>
          </perm-box>
            <a href='#'  v-if='record.type==="B"' @click='handleApply(record,false)'>停保详情</a>
        </span>
      </s-table>
    </a-card>
    <a-modal
    ...
    >
      ...
      <a-form :form='stusourceForm' >
        <a-form-item label='停保情况' :labelCol='{ span: 4 }' :wrapperCol='{ span: 14 }' >
          <a-radio-group  v-decorator="['stopType', { rules: [{ required: true, message: '请选择停保情况' }] }]">
            <a-radio :value="1">
              停保
            </a-radio>
          </a-radio-group>
        </a-form-item>
        <a-form-item label='停保月份' :labelCol='{ span: 4 }' :wrapperCol='{ span: 14 }' >
          <a-month-picker
            format='YYYY-MM'
            valueFormat='YYYY-MM'
            placeholder='请选择月份'
    ...
    >
      ...
      <div >
        <div class='mb20'>停保情况 <span class='ml20'>{{teacherInfo.socialSecurity===2?'已减员':''}}</span></div>
        <div class='mb20'>停保月份 <span class='ml20'>{{teacherInfo.effectiveDate?teacherInfo.effectiveDate.slice(0,7):''}}</span></div>
...
import { pageSocialSecurityStop, socialStop, socialDetail } from '@/api/education'
import Vue from 'vue'
  ...
  export default {
    ...
    data() {
      return {
        ...
        socialStatus:'A',
        loadData: parameter => {
          return pageSocialSecurityStop(Object.assign(parameter, this.queryParam,{type:this.socialStatus})).then(res => {
            return res
          })
        },
      }
    ...
    methods: {
      ...
      handleApply(record,type){
        ...
        if(type){
          ...
          const defaultStart = moment(date)
            .format('YYYY-MM')
          this.$nextTick(res=>{
            this.stusourceForm.setFieldsValue({ effectiveDate: defaultStart,stopType:1 ,remark:''})
          })
        }else{
      ...
      exportExcel() {
        ...
        form.action = `${process.env.VUE_APP_URL}/social/security/socialSecurityStopDown`
        form.method = 'POST'
