<template>
  <div class="studentinfo-wrapper">
    <a-card class="studentinfo-wrapper" :bordered="false" title="班级信息">
      <class-info-base-content @graduate="graduate" @actClass="actClass" :classInfo="classInfo" @complete="refreshPage"></class-info-base-content>
    </a-card>
        ...
        <a-tabs defaultActiveKey="1" @change="tabsChange">
          <a-tab-pane tab="学员情况" key="1" :forceRender="true">
            <class-stu-table
              ...
              @addStudent="addStudentBtn"
            ></class-stu-table>
          </a-tab-pane>
          <a-tab-pane v-if="handlePermBox('education:plan:view')" tab="课程安排" :key="2" :forceRender="true">
            <ClassInfoPlan ref="stuPlanTable" @signInStu="signInStu" :classInfo="classInfo" :isGraduate="isGraduate" :classId="classId"> </ClassInfoPlan>
          </a-tab-pane>
          <a-tab-pane tab="操作记录" :key="3" :forceRender="true">
            <operating-record :loadData="classChangeLog" ref="operatingRecord"></operating-record>
          </a-tab-pane>
          <a-tab-pane v-if="handlePermBox('education:class:view') /*&& isGraduate*/" tab="历史学员" :key="4" :forceRender="true">
            <HistoryStudentRecord :classId="classId" ref="HistoryStudentRecord"></HistoryStudentRecord>
          </a-tab-pane>
          <a-tab-pane v-if="handlePermBox('education:childclasslog:view') /*&& isGraduate*/" tab="升班记录" :key="5" :forceRender="true">
            <UpGrade :classId="classId" ref="upGrade"></UpGrade>
          </a-tab-pane>
        </a-tabs>
      </div>
    </a-card>
    <a-modal :width="800" :title="title" :visible="modalVisible" @ok="handleOk" :confirmLoading="modalConfirmLoading" @cancel="handleCancel">
      <class-info-modal-table :modalTableProps="modalTableProps" ref="modalRef" @hasSelected="hasSelected"></class-info-modal-table>
    </a-modal>
  </div>
</template>
<script>
import { getClassInfo, getClassStuList, saveStuToClass, graduateEduClass, actEduClass, classChangeLog } from '@/api/education'
...
import HistoryStudentRecord from '../modules/historyStudentRecord.vue'
import UpGrade from '../modules/upGrade.vue'
import ClassInfoPlan from '../modules/classInfoPlan.vue'
...
export default {
  ...
  methods: {
    ...
    tabsChange(e) {
      if (e === 4) {
        this.$refs.HistoryStudentRecord.getTable()
      }
      if (e === 5) {
        this.$refs.upGrade.getTable()
      }
    },
    //学生签到成功以后的回调
    signInStu() {
    ...
    actClass(id) {
      ...
      this.$confirm({
        ...
        cancelText: '取消',
        onOk() {
          _this._actClassApi(id)
        }
      })
    },
    _actClassApi(params) {
      actEduClass(params).then(res => {
        if (res.code === 200) {
          this.$notification['success']({
            message: '系统通知',
            description: '激活成功'
    ...
    handleOk(e) {
      ...
      if (flag) {
        ...
        saveStuToClass(classId, studentCardIds)
          .then(res => {
            if (res.code === 200) {
              this.$notification['success']({
                message: '系统通知',
