...
<script>
import { getServiceResourceContrastReport, exportServiceResourceContrastReport } from '@/api/echart/analysisChannel'
...
export default {
            ...
            if (this.isLastTotalRow(index)) {
              ...
              obj.children = (
                <div>
                  ...
                  {
                    <div class="table-cell-y text-weight-b background-grey" onClick>
                      {'合计'}
  ...
  methods: {
    // 先请求，后获取渠道
    fillColumns(firstTitles) {
      this.columns = this.columns.slice(0, 2)
      this.channelCols = []
      const secondTitles = [['净引流数', 'netDrainage'], ['资源数', 'resourcesNumber'], ['净转化率', 'conversionRate', 'bold']]

      firstTitles.forEach((firstTitle, firstIndex) => {
        const { channelName, channelId } = firstTitle
        this.channelCols.push(channelId)
        const firstCol = {
          title: channelName,
          children: []
        }
        secondTitles.forEach(arr => {
          const col = {
            title: arr[0],
            align: 'center',
            // width: 120,
            ...
            customRender: (value, record, index) => {
              return this.getDataRowCustomRender(arr[1], record, index, channelId)
            }
          }
    ...
    async loadData() {
      this.loading = true
      try {
        let res = await getServiceResourceContrastReport(this.queryParams)
        const { data } = res
        this.list = data.data
    ...
    getDataRowCustomRender(dataIndex, record, index, channelId) {
      const obj = {
        children: (
          <div>
            <div class="table-cell-y">{this.getLocaleNum(record.count[channelId][dataIndex])}</div>
          </div>
        ),
        attrs: {}
      }
      if (this.isLastTotalRow(index) && this.serviceNameOpen) {
        ...
        obj.children = (
          <div>
            {record.list.map((item, outerIndex) => {
              const listMap = item.list
              let rowData = ''
              this.channelCols.forEach((colChannelId, colIndex) => {
                // 单个渠道下${3}个参数的字段
                if (colChannelId === channelId) {
                  let channelObj = listMap[channelId]
                  if (channelObj) {
                    rowData = listMap[colChannelId][dataIndex]
                  }
                }
              })

              return <div class="table-cell-y">{this.getLocaleNum(rowData)}</div>
            })}
            {<div class="table-cell-y background-grey">{this.getLocaleNum(record.count[channelId][dataIndex])}</div>}
          </div>
        )
      }
      return obj
    ...
    async handleExport() {
      let res = await exportServiceResourceContrastReport(this.queryParams)
      this.$tools.exportExcel(res, '客服引流资源报名对比表')
    }
  }
}
</script>
