<template>
  <div class="audition-list" ref="elementAll" style="height:calc(100vh - 148px)">
    ...
    <a-modal :maskClosable="$store.state.modalMaskClickEnable" title="绑定报名" :visible="bindVisible"
...
import { getAuditionConversionNoFinancePageAuditionLog, auditionConversionUnbindAuditionLog, auditionConversionNoFinanceBindAuditionLog, bindPersonalAuditionRegister, unbindPersonalAuditionRegister, cancelAuditionlog } from '@/api/reception/student'
...
export default {
  ...
  data() {
    return {
      ...
      loadData: parameter => {
        return getAuditionConversionNoFinancePageAuditionLog(Object.assign(parameter, this.queryParam))
          .then(res => {
            return res
          })
      },
  ...
  methods: {
    ...
    handleUnBind(record) {
      this.$confirm({
        ...
        cancelText: '取消',
        onOk: () => {
          let ids = record.bindDetails.map(d => d.convertionId).join(',')
          auditionConversionUnbindAuditionLog(ids)
            .then(res => {
    ...
    handleBindOk() {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          auditionConversionNoFinanceBindAuditionLog(form)
            .then(res => {
              if (res.code == 200) {
                this.$notification['success']({
                  message: '系统通知',
