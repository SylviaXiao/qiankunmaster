    ...
    <a-card :bordered="false" :style="{ margin: '20px 0' }">
      ...
      <a-spin tip="加载中..." :spinning="spinning">
        <a-table
          ...
          :scroll="{ x: true }"
        >
          <span slot="singleEnrollCount" slot-scope="text, record, index, indent">
            <span v-if="record.deptArea == '合计'">{{(record.enroll199Count - record.tEnroll199Count).toFixed(2)}}</span>
            <a v-else @click="openSignInDetail(record, 'A', 'table1')">{{(record.enroll199Count - record.tEnroll199Count).toFixed(2)}}</a>
          </span>
          <span slot="tEnroll199Count" slot-scope="text, record, index, indent">
            <span v-if="record.deptArea == '合计'">{{text}}</span>
            <a v-else @click="openSignInDetail(record, 'B', 'table1')">{{text}}</a>
          </span>
      ...
      <a-spin tip="加载中..." :spinning="spinning">
        ...
        <span slot="singleEnrollCount" slot-scope="text, record, index, indent">
            <span v-if="record.deptArea == '合计'">{{(record.enroll199Count - record.tEnroll199Count).toFixed(2)}}</span>
            <a v-else @click="openSignInDetail(record, 'A', 'table2')">{{(record.enroll199Count - record.tEnroll199Count).toFixed(2)}}</a>
          </span>
          <span slot="tEnroll199Count" slot-scope="text, record, index, indent">
            <span v-if="record.deptArea == '合计'">{{text}}</span>
            <a v-else @click="openSignInDetail(record, 'B', 'table2')">{{text}}</a>
          </span>
          <span slot="number" slot-scope="text, record, index, indent">
            <span v-if="Number(text || 0) === 0">{{text}}</span>
            <a v-else @click="toDetail(record, indent)">{{text}}</a>
      ...
      <a-spin tip="加载中..." :spinning="spinning">
        ...
        <span slot="singleEnrollCount" slot-scope="text, record, index, indent">
            <span v-if="record.deptArea == '合计'">{{(record.enroll199Count - record.tEnroll199Count).toFixed(2)}}</span>
            <a v-else @click="openSignInDetail(record, 'A', 'table3')">{{(record.enroll199Count - record.tEnroll199Count).toFixed(2)}}</a>
          </span>
          <span slot="tEnroll199Count" slot-scope="text, record, index, indent">
            <span v-if="record.deptArea == '合计'">{{text}}</span>
            <a v-else @click="openSignInDetail(record, 'B', 'table3')">{{text}}</a>
          </span>
          <span slot="number" slot-scope="text, record, index, indent">
            <span v-if="Number(text || 0) === 0">{{text}}</span>
            <a v-else @click="toDetail(record, indent)">{{text}}</a>
...
<script>
  ...
  import { totalPrivateEduAuditionCourseEnrollMasterReport } from '@/api/organize'
  import signInDetail from './signInDetail.vue'
    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          align: 'center',
          width: 100,
          scopedSlots: { customRender: 'singleEnrollCount' }
        },
        {
          title: '套报总人数（199+非私教）',
          dataIndex: 'tEnroll199Count',
          align: 'center',
          width: 100,
          scopedSlots: { customRender: 'tEnroll199Count' }
        }
      ]
    },
    {
      ...
      children: [
        ...
        {
          title: '邀约率',
          dataIndex: 'danceCount.invitationRate',
          align: 'center',
          width: 100
        },
        {
  ...
  export default {
    ...
    methods: {
      ...
      initTable() {
        this.spinning = true
        totalPrivateEduAuditionCourseEnrollMasterReport(Object.assign({}, this.queryParam))
          .then(res => {
      ...
      toDetail(record, indent) {
        ...
        // localStorage.setItem('personalAuditionRegisterReport' + params.detailType, JSON.stringify(queryParam))
      ...
      downloadStu() {
        let exportUrl = '/auditionCourseTotalEnroll/privateEduAuditionCourseEnrollMasterReportByExport'
      ...
      openSignInDetail(record, type, tableName) {
        const extData = {
          type,
        ...
        this.$refs.signInDetail.open(record, extData)
      }
    }
  }
</script>
