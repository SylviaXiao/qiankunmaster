<template>
  <div>
    ...
    <a-card :bordered="false">
      <div class="mb20">
        <perm-box perm="finance:totaltask-target:save">
          <a-button type="primary" icon="plus-circle" @click="handleAdd">总任务目标录入</a-button>
        </perm-box>
      </div>
      <s-table
        ref="table"
      ...
      >
        <span slot="action" slot-scope="text, record">
          ...
          <perm-box perm="finance:totaltask-target:del">
    ...
    >
      ...
      >
        ...
        <span slot="targetNumber" slot-scope="text, record">
          <a-input-number v-model="record.targetNumber" :min="0" style="width: 100%;" />
        </span>
...
<script>
  ...
  import {  listTotalTaskTarget, removeTotalTaskTarget, saveOrUpdate } from '@/api/organize'
  ...
  const formColumns = [
    ...
    },
    {
      ...
      dataIndex: 'targetNumber',
      width: 100,
      scopedSlots: { customRender: 'targetNumber' }
    },
    {
      title: '操作',
      dataIndex: 'action',
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        columns,
        loadData: parameter => {
          return listTotalTaskTarget(Object.assign({}, this.queryParam, parameter)).then(res => {
            return res
          })
        },
        visible: false,
    ...
    methods: {
      ...
      initForm(record) {
        this.form = []
        if (record) {
          const { id, deptId, yearMonth, targetNumber } = record
          this.form.push({
            id,
            yearMonth,
            orgDept: { id: deptId },
      ...
      handleOk() {
        let form = this.form
        //   .filter(item => item.targetNumber !== 0)
        // if (form.length === 0) {
