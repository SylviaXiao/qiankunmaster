  ...
  <div ref="elementAll" style="height:calc(100vh - 84px)">
    ...
    <a-card :bordered="false">
      <div class="btnGroup" style="margin-bottom: 15px;" ref="elementBtn">
        <perm-box perm="finance:spending:save">
          <a-button type="primary" @click="expendEnter()">财务收入录入</a-button>
        </perm-box>
        ...
        <perm-box style="margin-left:20px;" perm="finance:spending:export">
          <a-button type="primary" icon="download" @click.native="downloadStu">
            导出
          </a-button>
        </perm-box>
        <perm-box style="margin-left:20px;" perm="finance:splitproject:view">
          <a-button type="primary" @click="danceClassType">
            舞种班型导出
          </a-button>
        </perm-box>
      </div>
      <perm-box perm="finance:spending:view">
        <s-table
          :showSlider="true"
          ref="table"
          size="default"
        ...
        >
          ...
          <span slot="action" slot-scope="text, record">
            <perm-box perm="finance:spending:update || finance:spending-social:update">
              <a href="javascript:;" @click="edit(record)" v-if="!record.isSplit">修改</a>
            </perm-box>
            <perm-box perm="finance:spending:del">
    ...
    <expenditureEnter ref="expenditure" @refresh="loadTable"></expenditureEnter>
...
import { listOrgDept, pageFinSpending, removeFinSpendingItem, getBankList, getAllSysFeeItem } from '@/api/education/card'
import expenditureEnter from './modules/expenditureEnter.vue'
...
export default {
  ...
  data() {
    return {
      ...
      defaultLayout,
      loadData: parameter => {
        return pageFinSpending(Object.assign(parameter, this.queryParam, { incType: 'K' })).then(res => {
          return res
        })
      },
      searchParams: [
        ...
        // splitDate splitDeptName feeName feeItemName  deptName  date price price remark
        {
          type: 'cascader',
          key: 'costName',
          label: '成本归类',
  ...
  methods: {
    ...
    //导出
    downloadStu() {
      let exportUrl = '/finance/spending/pageFinSpendingByExportExcel'
    ...
    //支出录入
    expendEnter() {
      this.$refs.expenditure.open(null, null, 'K', '收入录入')
    },
    //社保工资录入
    socialSecurity() {
    ...
    delect(record) {
      this.$confirm({
        ...
        cancelText: '取消',
        onOk: () => {
          removeFinSpendingItem(record.id).then(res => {
