...
import { listOrgDept, listFeeName, listBank, listFeeItem, saveFiscalCharges, getFinSpendingItem, updateFiscalChargesFinance } from '@/api/education/card'
        ...
        if (valid) {
          for (let i = 0; i < this.tableData.length; i++) {
            if (this.tableData[i].finSpendingItemSplit && this.tableData[i].finSpendingItemSplit.length) {
              ...
              if (this.tableData[i].price !== Number(totalPrice.toFixed(2))) {
                return this.$notification['error']({
                  message: '系统通知',
          ...
          let request = saveFiscalCharges
          if (this.finSpendingId) {
            request = updateFiscalChargesFinance
          }
