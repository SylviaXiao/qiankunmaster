  ...
  <div ref="elementAll" style="height:calc(100vh - 84px)">
    ...
    <a-card :bordered="false">
      <div class="btnGroup" style="margin-bottom: 15px;text-align: right;" ref="elementBtn">
        <perm-box perm="finance:secspl:updatespl">
          <a-button type="primary" @click="secondSplit()">修改分摊</a-button>
        </perm-box>
        ...
        >
          <span slot="finSecondSplits" slot-scope="text, record">
              <a-tooltip v-if="text.length>0" trigger='click'>
                <template slot="title">
                  <div v-for='(item ,index) in text' :key='index'>{{item.deptName}}：{{item.secPrice}}</div>
                </template>
          ...
          <span slot="action" slot-scope="text, record">
            <perm-box perm="finance:secspl:updatespl" >
              <a href="javascript:;" @click="editRules(record)" >修改分摊金额</a>
            </perm-box>
          </span>
        </s-table>
    </a-card>
...
export default {
  ...
  methods: {
    //导出
    downloadStu() {
      let exportUrl = '/finance/secspl/downSecSpl'
